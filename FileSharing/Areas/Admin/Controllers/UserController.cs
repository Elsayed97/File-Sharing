...
namespace FileSharing.Areas.Admin.Controllers
{
    public class UserController : AdminBaseController
    {
        ...
        public async  Task<IActionResult> Block(string userId)
        {
            ...
            {
                var result = await userService.ToogleBlockUserAsync(userId);
                if (result.Success)
                {
        ...
        {
            var totalUsersCount = await userService.UserRegisterationCountAsync();
            var month = DateTime.Today.Month;
            var monthUsersCount = await userService.UserRegisterationCountAsync(month);
